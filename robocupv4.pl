...
can_shoot(Team, X, Y) :-
    ...
    Distance =< 45,  % Slightly increased shooting range
    \+ (
...
shoot_ball(Team, Role) :-
    ...
    NewBallX is X1 + (DX * KickPower // 15) + RandomX,  % Increased shot power
    NewBallY is Y1 + (DY * KickPower // 15) + RandomY,
...
goal_scored(Team) :-
    ...
    update_score(Team),
    reset_ball,
    !.  % Cut to prevent backtracking after a goal
...
process_ball_actions :-
    % First check for goals
    (goal_scored(team1) ; goal_scored(team2) ; true),
    !,  % Cut to prevent multiple goal checks
