...
shoot_ball(Team, Role) :-
    player(Team, Role, position(X1, Y1), _, _, Attacking, _), % 7 arguments
    ball(position(X2, Y2)),
    (Team = team1 -> goal_position(team2, GoalX, GoalY) ; goal_position(team1, GoalX, GoalY)),
...
goalkeeper_actions(Team) :-
    ...
    ball(position(BX, BY)),
    % Get the last shot power (we'll need to track this)
    (game_state(last_shot_power, Power) -> true ; Power = 30), % Default if not set
    
    % Calculate save probability based on shot power and goalkeeper skill
    % Base save chance is 75% (like the original 25% failure chance)
    % Higher shot power reduces this chance
    BaseSaveChance is 75 + (Defending // 2), % Better goalkeepers have higher base chance
    PowerFactor is max(10, 50 - (Power // 2)), % Higher power reduces chance
    SaveChance is (BaseSaveChance * PowerFactor) // 50, % Scale to 0-100 range
    
    % Random roll to determine if save is successful
    random(100) < SaveChance,
    !, % Cut to commit to this choice
    format('~w goalkeeper makes a fantastic save!~n', 
           [Team]),
    goalkeeper_kick(Team).

goalkeeper_actions(Team) :-
    % If we get here, the save failed
