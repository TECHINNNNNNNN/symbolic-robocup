...
goal_scored(Team) :-
    ...
    % Fetch and update the score for the scoring team
    ...
    ;
        ...
        assertz(game_state(score(Team, 1)))
    ),
    
    % Retrieve and display the updated scores
    findall(S1, game_state(score(team1, S1)), Team1Scores),
    findall(S2, game_state(score(team2, S2)), Team2Scores),
    last(Team1Scores, Score1),
    last(Team2Scores, Score2),
    
    format('GOAL! ~w scores! Current score: team1 ~w - team2 ~w~n', 
           [Team, Score1, Score2]),
    reset_positions,
    reset_ball.
